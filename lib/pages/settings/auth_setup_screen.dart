...
class _AuthSetupScreenState extends State<AuthSetupScreen> {
  ...
  Future<void> _checkBiometricAvailability() async {
    ...
    setState(() {
      ...
      _useBiometric = isAvailable; // Par défaut, utiliser la biométrie si disponible
    });
  }
  ...
  Future<void> _setupAuth() async {
    ...
    if (_pinController.text.length < 4) {
      context.showWarningSnackBar('Le code PIN doit contenir au moins 4 chiffres');
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Configuration de la sécurité', style: Theme.of(context).textTheme.titleMedium),
      ),
      body: SafeArea(
